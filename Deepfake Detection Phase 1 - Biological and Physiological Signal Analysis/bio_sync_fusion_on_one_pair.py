...
class BioSyncFusion:
    ...
    def run_analysis(self, video_path: str, label: str = "Unknown", max_frames: int = 300, resize_width: int = 480):
        ...
        # --- Basic correlation metrics (NO POLARITY FLIP) ---
        corr_coef_raw, lag_seconds = self.compute_correlation_and_lag(rppg, motion, fps)
        # use absolute correlation as feature but keep lag info separately
        abs_corr = abs(corr_coef_raw)

        # compute coherence using raw signals
        ...
        # HRV variability (only if clip long enough)
        hrv = self.compute_hrv(rppg, fps)

        # sliding-window sync correlation
        local_corr = self.sliding_corr(rppg, motion)

        # True spectral coherence
        coh_true = self.true_coherence(rppg, motion, fps)

        # ==========================================================
        # Ridge strength & SNR (your original code)
        ridge_power = np.mean(np.max(power_rppg, axis=0)) if power_rppg.size else 0.0
        median_power = np.median(power_rppg) if power_rppg.size else 1.0
        ridge_strength = float(ridge_power / (median_power + 1e-8))
        ...
        # Dominant frequency summary (use median of per-frame peaks)
        ...
        freq_diff = abs(dom_freq_rppg - dom_freq_motion)  # Hz

        # ==========================================================
        # Improved Fusion Logic (minimal change, robust rules)
        # 1) If frequency difference is large (> 0.2 Hz), flag fake immediately
        # 2) Otherwise compute a composite score (no polarity flip, no noisy HRV unless clip long)
        # ==========================================================
# ==========================================================
        #  FIXED FUSION LOGIC: Trust Correlation + Phase Locking
        # ==========================================================
        
        # 1. Check Phase Locking (Real Life Sign)
        # If signals are highly correlated and time-aligned, it's REAL,
        # even if frequency calculation is slightly noisy (e.g. 0.29 vs 0.20)
        ...
        f_snr = np.clip(snr_rppg, 0.0, 1.0)
        f_hrv = 0.0 if hrv == 0.0 else np.clip(1.0 - min(hrv, 120)/120.0, 0.0, 1.0)

        # 3. Soft Frequency Penalty (instead of Hard Gate)
        # Map 0.0 diff -> 1.0 score, 0.5 diff -> 0.0 score
        f_freq = np.clip(1.0 - (freq_diff / 0.5), 0.0, 1.0)

        if is_phase_locked:
            # TRUST CORRELATION heavily
            fusion_score = (
                0.40 * f_corr +
                0.20 * f_freq +
                0.20 * max(0.0, f_local) +
                0.10 * f_coh + 
                0.10 * f_roi
            )
            # Boost because phase locking is rare in fakes
            fusion_score = min(fusion_score + 0.15, 1.0)
        else:
            # NO LOCK -> TRUST FREQUENCY
            fusion_score = (
                0.25 * f_corr +
                0.40 * f_freq +
                0.15 * max(0.0, f_local) +
                0.10 * f_coh + 
                0.10 * f_roi
            )
            # Penalty for bad sync
            fusion_score = max(fusion_score - 0.1, 0.0)
        is_fake = fusion_score < 0.4
        verdict = "FAKE" if is_fake else "REAL"

        print(f"Pearson correlation (rPPG vs BCG): {corr_coef_raw:.3f} (abs={abs_corr:.3f})")
        print(f"Estimated lag (s): {lag_seconds:.3f}")
        print(f"Mean coherence in HR band: {phase_coh:.3f}")
        print(f"Dominant freq rPPG: {dom_freq_rppg:.3f} Hz, motion: {dom_freq_motion:.3f} Hz, diff: {freq_diff:.3f} Hz")
        print(f"Fusion score: {fusion_score:.3f} -> Verdict: {verdict}")

        # --- Plotting (unchanged visual diagnostics) ---
        ...
        axs[0].set_title(f"{label}: Signals (Corr={corr_coef_raw:.2f}, Local={local_corr:.2f}, CohTrue={coh_true:.2f})")
        axs[0].legend()

        ax1 = axs[1]
        im1 = ax1.imshow(power_rppg, aspect='auto', origin='lower',
                         extent=[0, len(rppg)/fps, freqs_rppg[0], freqs_rppg[-1]])
        ax1.set_title("rPPG Wavelet Power (CWT - Morlet)")
        fig.colorbar(im1, ax=ax1, orientation='horizontal')

        ax2 = axs[2]
        im2 = ax2.imshow(power_motion, aspect='auto', origin='lower',
                         extent=[0, len(motion)/fps, freqs_motion[0], freqs_motion[-1]])
        ax2.set_title("BCG Wavelet Power (CWT - Morlet)")
        ...
        # ---------------------------------------------------------
        # FORENSIC EXPLAINABILITY FRAME GENERATOR (REUSES landmarks_history)
        # ---------------------------------------------------------
        print("Generating forensic explainability video (frequency-difference overlay)...")
        explain_frames = []
        snippet_len = min(150, len(frames))

        # compute per-frame dominant freq arrays (instantaneous)
        ...
        if n_inst > 0:
            ...
            # normalize for heatmap: 0..0.4 Hz -> 0..1
        ...
        for i in range(snippet_len):
            ...
            # FIX: reuse stored landmarks_history rather than re-running face detector
            lm = landmarks_history[i] if i < len(landmarks_history) else None

            # lm = landmarks_history[i]   # Already aligned
            ...
            # 1. Draw a Top Bar for the Score (Clean Background)
            # Height 45px to fit 2 lines of text
            cv2.rectangle(frame, (0,0), (frame.shape[1], 45), color, -1) 
            
            tag = "REAL" if fusion_score >= 0.4 else "FAKE"
            color_label = (0,255,0) if tag=="REAL" else (0,0,255)
            
            # 2. Draw Text (Smaller Scale, Better Position)
            # Line 1: Verdict + Score
            cv2.putText(frame, f"{tag} ({fusion_score:.2f})", (5, 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2, cv2.LINE_AA)
            
            # Line 2: Freq Diff (Debug info)
            cv2.putText(frame, f"dF: {freq_diff:.2f}Hz", (5, 40),
        ...
        # --- Summary ---
        print("\n=== Bio-Sync-Fusion Summary ===")
        print(f"Frames processed: {len(frames)} | FPS: {fps:.2f}")
        print(f"ROI consistency: {roi_consistency:.3f}")
        print(f"HRV: {hrv:.3f}")
        print(f"Local corr: {local_corr:.3f}")
        print(f"True coherence: {coh_true:.3f}")
        print(f"Fusion score: {fusion_score:.3f} (threshold=0.4)")
...
if __name__ == "__main__":
    ...
    physio_fusion_net = BioSyncFusion()
    if os.path.exists(real_vid):
        physio_fusion_net.run_analysis(real_vid, "REAL_VIDEO", max_frames=300, resize_width=480)
    else:
        print(f"File not found: {real_vid}. Please upload '{real_vid}' to test.")

    if os.path.exists(fake_vid):
        physio_fusion_net.run_analysis(fake_vid, "FAKE_VIDEO", max_frames=300, resize_width=480)
    else:
        print(f"File not found: {fake_vid}. Please upload '{fake_vid}' to test.")
